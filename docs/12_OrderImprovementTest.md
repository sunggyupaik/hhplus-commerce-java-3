...
테스트를 기반으로 가상의 장애보고서를 작성했습니다.

|제목|신상품 주문 데드락 오류 |
|---|---|
|  작성자 |  백성규 선임 |
| 소속  | 항해플러스 이커머스 3팀  |
| 작성일시  | 2024년 11월 28일 17:00  |
| 장애시간 | 2024년 11월 28일 13:00 ~ 14:00  |
| 장애위치 | 이커머스 3팀 신상품 주문 API  |
| 장애현상 | 주문화면에서 다음 단계로 넘어가지 못하고 화면이 멈춘 고객들 다수 발생  |
| 영향도 |  13:00 신상품 오픈 직후 주문 요청 건들 중 주문 완료 실패율 54.84% 발생 |
| 장애원인 | 신상품 주문 폭주로 DB 트랜잭션 데드락 발생 |
| 장애피해 조치 | 데드락에 걸린 트랜잭션 강제 종료  및 서버 재기동 및 서비스 정상(~14:00) |
| 향후 대책 | 스트레스 테스트를 통해 병목현상 확인하여 개선 <br> 1. 주문 시 상품 정렬하여 데드락 회피 <br> 2. 인기상품 재고 Redis 관리로 DB 부하 감소 <br> 3. 인기상품 오픈 시, 서버 증설 및 모니터링 강화 |
| 기타 | 선착순 주문 요청한 고객들에게 사과 알림톡 발송 및 10%쿠폰 지급 |
